...
export default function App() {
  ...
  const handleLetterPress = (letter, sideIndex, letterIndex) => {
    ...
    if (
      usedSides.length === 0 ||
      usedSides[usedSides.length - 1].sideIndex !== sideIndex
    ) {
      ...
      setStatusMessage(
        "Can't use two letters from the same side consecutively."
      );
    }
  };
  ...
  // find which letter was last typed or used in previous word
  const currentHighlight =
    currentWord.length > 0
      ? usedSides[usedSides.length - 1]
  ...
  const handleSubmit = () => {
    ...
    setCompletedPaths([...completedPaths, [...usedSides]]);
    ...
    const uniqueUsedLettersCount = new Set(
      newUsedLetters.map((used) => `${used.sideIndex}-${used.letterIndex}`)
    ).size;
    ...
    if (uniqueUsedLettersCount === totalLetters) {
      setStatusMessage(
        `You won! All letters used in ${wordsAcceptedCount + 1} words!`
      );
    } else {
  ...
  useEffect(() => { // keyboard function
    const handleKeyDown = (event) => {
      const key = event.key.toUpperCase();

      // check for A-Z, backspace, and enter
      if (/^[A-Z]$/.test(key)) {
        // check if this letter is in the graph
        const match = letters.find((l) => l.letter === key);

        if (match) {
          handleLetterPress(match.letter, match.sideIndex, match.letterIndex);
        } else { // if not, issue status
          setStatusMessage(`"${key}" is not in the puzzle.`);
        }
      }

      // check for backspace to remove last letter
      else if (event.key === "Backspace") {
        setCurrentWord((prev) => prev.slice(0, -1));
        setUsedSides((prev) => prev.slice(0, -1));
      }

      // check for enter to submit the current word
  ...
  }, [letters, handleLetterPress, handleSubmit]);
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.rightPanel, { width: rightWidth }]}>
          ...
          {letters.map((item, idx) => {
            ...
            // when printing a letter, check if it is the current highlight
            const isHighlighted =
              currentHighlight &&
              currentHighlight.sideIndex === item.sideIndex &&
              currentHighlight.letterIndex === item.letterIndex;
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.letter,
                  {
                    ...
                    backgroundColor: isHighlighted ? "#faa6a4" : isUsed ? "#d6cece" : "#ffffff",
                    borderColor: "#000000",
                    transform: [{ scale: pressed ? 0.9 : 1 }], // scale up 10% when pressed
                  },
                ]}
              >
                <Text style={[styles.letterText, isHighlighted && { color: 'white' }]}>
                  {item.letter}
                </Text>
              </Pressable>
            );
